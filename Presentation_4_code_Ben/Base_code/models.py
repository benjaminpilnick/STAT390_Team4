...
class HierarchicalAttnMIL(nn.Module):
    ...
    def __init__(
        ...
        embed_dim: int = 512,
        #dropout: float = 0.3,
        # Use classifier dropout and patch projection dropout separately based on OPTUNA reuslts 
        classifier_dropout: float = 0.3, #Means we will get it from the OPTUNA_HYPERPARAMS dict in config.py
        patch_proj_dropout: float = 0.3, #Means we will get it from the OPTUNA_HYPERPARAMS dict in config.py
        pooled_dim: int = 4096,
    ):
        super().__init__()

...
# -----------------------------------------------------------------------------
SEARCH_SPACE = {
    "learning_rate": {"low": 3e-5, "high": 1e-4, "log": True},
    "weight_decay": {"low": 1e-5, "high": 5e-4, "log": True},
    "patch_proj_dropout": {"low": 0.45, "high": 0.65, "log": False},
    "classifier_dropout": {"low": 0.10, "high": 0.40, "log": False},
    "class_weight_benign": {"low": 1.5, "high": 3.0, "log": False},
    "entropy_lambda": {"low": 1e-5, "high": 1e-3, "log": True},
}
...
#Creating 16 sobol points for the 6 hyperparameters to be used as the startup trials for OPTUNA. Then 
# it will use TPE. The Key point here is the map_unit_to_range function which takes the unit cube samples
# and maps them to the actual hyperparameter ranges, applying the log if necessary. 
...
def suggest_hparams(trial: optuna.Trial) -> Dict[str, float]:
    return {
        "learning_rate": trial.suggest_float("learning_rate", 3e-5, 1e-4, log=True), #shifted down 
        "weight_decay": trial.suggest_float("weight_decay", 1e-5, 5e-4, log=True), #shifted down
        "patch_proj_dropout": trial.suggest_float("patch_proj_dropout", 0.45, 0.65), #shift up to 0.70
        "classifier_dropout": trial.suggest_float("classifier_dropout", 0.10, 0.40), #shift down 
        "class_weight_benign": trial.suggest_float("class_weight_benign", 1.5, 3.0), #shift down. 
        "entropy_lambda": trial.suggest_float("entropy_lambda", 1e-5, 1e-3, log=True), #unchanged
    }
